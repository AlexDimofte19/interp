...
#                              to look. This one measures it everywhere, which is what a
#                              loudness-vs-probe analysis over ALL tokens needs. It is not in
#                              the default arm list -- it is ~11x the cutoffs per trajectory and
#                              is meant to be run over a smaller, held-out trajectory set.
#
...
# This arm loads gpt-oss-20b, so it needs the `gpu` extra (accelerate, and the pinned
# kernels==0.12.0). A checkout whose .venv predates the extra already has accelerate and
# will not notice; a fresh worktree venv dies at from_pretrained without it.
...
for STRATEGY in "${STRATEGIES[@]}"; do
    ...
    set +e
    uv run --project "$REPO" $UV_EXTRAS python "$HERE/run_inference.py" \
        --strategy "$STRATEGY" \
