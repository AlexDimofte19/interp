...
Three strategies, selected by name:
...
def build_strategy(
    ...
    *,
    ...
    """Instantiate a strategy by name, wiring loudness only for the ones that need it.

    >>> build_strategy("eos").name
    'eos'
    >>> sorted(STRATEGIES)
    ['eos', 'jlens_argmax_per_sentence', 'jlens_top_k_global']
    """
    try:
        cls = STRATEGIES[name]
    except KeyError:
        raise ValueError(f"unknown strategy {name!r}; choose one of {sorted(STRATEGIES)}") from None
    if not cls.needs_loudness:
        return cls()
