...
def resolve_trajectory_paths(args, expand_paths: Callable[[list[str]], list[str]]) -> list[str]:
    ...
    if args.names_file:
        ...
        print(
            f"--names-file {args.names_file}: {len(keep)} name(s) -> kept {len(paths)} of {before} file(s)", flush=True
        )
...
def has_work(paths: list[str], args) -> bool:
    """True when there is something to gather; False when there legitimately is not.

    A finished ``--per-combo`` run has nothing left -- every cell is full and ``is_done``
    filtered it away -- and that is a normal resume, not a failure. Anything else means the
    filters removed everything, which is a config error (a mistyped ``--names-file``, most
    likely) and must NOT exit 0 looking like a successful gather: this repo has already been
    bitten once by a script that "succeeded" while writing nothing.
    """
    if paths:
        return True
    if args.per_combo is not None:
        print("every requested size x complexity cell is already complete; nothing to do", flush=True)
        return False
    raise ValueError(
        f"no trajectories left to process from {args.trajectory_paths} after --names-file/--sizes/--complexities"
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--names-file",
        default=None,
        help="File of trajectory stems (whitespace-separated) to KEEP, applied before every "
        "other filter. This is how a second tree is made to cover the same trajectories as an "
        "existing one; --per-combo/--seed do not reproduce a previous draw.",
    )
    ap.add_argument(
        "--sizes",
        default=None,
