#!/usr/bin/env bash
# gpt-oss P2 GRID, step 5: train one BINARY grid-cell probe per arm x cell class x model type.
#
# The grid twin of wrappers/qwen_analysis/3_train_and_eval_probes/train_next_action_probes_all_selections.sh.
...
#
# NO SPLIT: --eval-data-path is the mass-era 720, disjoint from the 2,880 by construction, so
# no trajectory is in both halves and the trainer's internal row-level split never runs.
#
...
# existing grid_binary_l15 probes rather than beside the next_action ones. Pinned, not
...
for arm in $ARMS; do
    for cls in $CLASSES; do
        for mt in $MODEL_TYPES; do
            out="${PROBES}/gptoss_p2_grid_${arm}_${cls}_l${LAYER}_${mt}.pt"
            if [ -f "$out" ]; then
...
echo
echo "done -> $PROBES/gptoss_p2_grid_<arm>_<class>_l${LAYER}_<type>.pt   logs -> $LOGS"
