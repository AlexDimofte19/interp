#!/usr/bin/env bash
# gpt-oss P2 GRID, step 5b: train one MULTICLASS grid-cell probe per arm x model type.
#
# The gpt-oss twin of wrappers/qwen_analysis/grid/3_train_and_eval_probes/train_grid_probes_all_selections.sh,
# and the multiclass twin of ./train_binary_grid_probes_all_selections.sh: same manifests
...
# --class-weight balanced is what keeps A and G (~0.5% of rows each) from being ignored.
#
...
# at large. ../4_loudness_evaluation scores every probe on every held-out token.
#
# NO SPLIT: --eval-data-path is the mass-era 720, disjoint from the 2,880, so no trajectory is
# in both halves. (The trainer refuses an internal --eval-split on these token-major manifests.)
#
...
for arm in $ARMS; do
    for mt in $MODEL_TYPES; do
        out="${PROBES}/gptoss_p2_grid_${arm}_multiclass_l${LAYER}_${mt}.pt"
        if [ -f "$out" ]; then
...
echo
echo "done -> $PROBES/gptoss_p2_grid_<arm>_multiclass_l${LAYER}_<type>.pt   logs -> $LOGS"
