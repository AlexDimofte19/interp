#!/usr/bin/env bash
# gpt-oss P2, dataset creation step 3c: ROLLOUT + RELABEL for the RANDOM control arm -- train and val.
#
# The gpt-oss twin of wrappers/qwen_analysis/2_dataset_creation/3_rollouts/rollout_random.sh.
# Stages 2 and 3 of three. ../2_preparations/prepare_random.sh must have run: this reads the
...
#
# LENS IS A LABEL HERE, NOT A CHOOSER. The control has no ruler of its own. jlens is named only
# so dir_logmass is on the same scale as the jlens arm's. It moves no cutoff: the picks come
# from the record, which lives in the jlens tree (the control was drawn there, before pruning).
#
...
TREE=/workspace/activations/jlens_mass_l15   # its selection record and its mass tables
