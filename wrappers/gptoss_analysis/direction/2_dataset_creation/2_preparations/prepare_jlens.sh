#!/usr/bin/env bash
# gpt-oss P2, dataset creation step 2a: the PREPARE stage for the JLENS arm -- train and val.
#
# The gpt-oss twin of wrappers/qwen_analysis/2_dataset_creation/2_preparations/prepare_jlens.sh.
# Stage 1 of three: token-major manifests labelled with the trajectory's final agent_action.
# ../3_rollouts/rollout_jlens.sh measures the local belief at those same tokens and swaps it
...
# tree it lives in). Here: the 20 tokens per trajectory the Jacobian lens scored loudest at
# layer 15, ranked by logprob_mass_full over the whole chain.
#
