#!/usr/bin/env bash
# Qwen P2 GRID: score each MULTICLASS grid probe on every held-out token (held-out 72), one JSON per probe.
#
...
DATA=/workspace/prepared/qwen_p2_grid_heldout   # built by prepare_heldout_grid.sh
...
echo
echo "=== held-out 72: balanced accuracy, no padding (incl. padding) ==="
for mt in $MODEL_TYPES; do
    line="  multiclass/${mt}:"
    for arm in $ARMS; do
