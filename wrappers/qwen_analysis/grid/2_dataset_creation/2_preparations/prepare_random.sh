#!/usr/bin/env bash
# Qwen P2 GRID, dataset creation step 2c: the grid_tile PREPARE stage for the RANDOM control arm -- train and val.
#
# The grid twin of ../../../2_dataset_creation/2_preparations/prepare_random.sh. The label is
...
# {stem}_jlens_selection.json are read. Here: the matched CONTROL, 60 tokens drawn uniformly
# from the same 20% sample at gather time, with the same N and seed as the lens arms. Its
# draw cannot be re-made; recorded_random is the only way to read it back.
#
