#!/usr/bin/env bash
# Qwen P2 GRID, step 6b: the cross-selection matrix for the MULTICLASS grid probes -- every
# probe from ./train_grid_probes_all_selections.sh on every arm's val slice.
# 3 slices x 3 probes x {lr, mlp} = 18 cells.
#
# The grid twin of ../../3_train_and_eval_probes/cross_eval_local_belief_all_selections.sh, cell
# for cell: telos_interp/loudness_analysis/eval_grid_probe.py replaces eval_local_belief.py
...
#
# THE SIGNAL VOCABULARY IS QWEN'S GRID ONE, the vocabulary the selection used. The split at
# the bottom of each cell -- is the token itself a grid word? -- means nothing against another.
#
# CAVEAT, 2026-09-23: the jlens and logitlens slices were SELECTED with the wrong-norm Qwen
# lens (`* w`, not `* (1 + w)`; claude_session_readme.md, last section). The random slice and
# every label are unaffected.
#
...
PREPARED=/workspace/prepared/qwen_p2_grid          # reads ${PREPARED}_${slice}_val
OUT=/workspace/results/qwen_p2_grid/cross_selection_eval_multiclass
...
for slice in $SLICES; do
    ...
    for arm in $ARMS; do
        for mt in $MODEL_TYPES; do
            probe="${PROBES}/qwen_p2_grid_${arm}_multiclass_l${LAYER}_${mt}.pt"
