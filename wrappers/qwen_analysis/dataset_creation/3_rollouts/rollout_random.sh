#!/usr/bin/env bash
# Qwen P2, dataset creation step 3c: ROLLOUT + RELABEL for the RANDOM control arm -- train and val.
#
...
# cut points are exactly the tokens its probe trains on and nothing else is rolled out. For
# the control that is not a convenience but the only option: its uniform draw was taken
# before the tree was pruned and can never be re-made, so it has to be read back.
#
