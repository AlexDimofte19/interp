...
# raising io-workers past its default of 16 makes it worse. The 2026-09-17 run took
# 8 h 40 m wall (11:29 -> 20:09 UTC) for all 1,169,734 files: 37.5 files/s averaged, the
# BOTTOM of that range rather than the top, drifting between 34 and 41 over the run. So
# budget ~9 h, not the ~3.5 h the 100 files/s figure suggests. Throughput tracked
# wall-clock, NOT chain length -- a 5k-token chain and a 34k-token one an hour apart ran
# at the same tokens/s -- so it is contention on the shared MooseFS cluster, and this
# number will not reproduce. Kept at 1.0 anyway: the argument above is about what the
# analysis needs, and that has not changed.
#
# Do NOT edit this file while a run launched from it is still going: bash reads a script
# by byte offset, so changing its length mid-run makes the shell resume mid-word. That
# happened here and cost a spurious "line 91: s: command not found" after a clean finish.
#
